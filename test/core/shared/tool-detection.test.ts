...
describe('tool-detection', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `clearspec-test-${randomUUID()}`);
  ...
  describe('SKILL_NAMES', () => {
    it('should contain all skill names matching COMMAND_IDS', () => {
      ...
      expect(SKILL_NAMES).toContain('clearspec-apply-change');
      expect(SKILL_NAMES).toContain('clearspec-ff-change');
      expect(SKILL_NAMES).toContain('clearspec-sync-specs');
      expect(SKILL_NAMES).toContain('clearspec-archive-change');
      expect(SKILL_NAMES).toContain('clearspec-bulk-archive-change');
  ...
  describe('getToolSkillStatus', () => {
    ...
    it('should detect when one skill exists', async () => {
      const skillDir = path.join(testDir, '.claude', 'skills', 'clearspec-explore');
...
`);
  ...
  describe('getConfiguredTools', () => {
    ...
    it('should return configured tools', async () => {
      ...
      // Setup Cursor
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'clearspec-explore');
