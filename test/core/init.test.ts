...
describe('InitCommand', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `clearspec-init-test-${Date.now()}`);
    await fs.mkdir(testDir, { recursive: true });
    originalEnv = { ...process.env };
    // Use a temp dir for global config to avoid reading real config
    configTempDir = path.join(os.tmpdir(), `clearspec-config-init-${Date.now()}`);
  ...
  describe('execute with --tools flag', () => {
    it('should create ClearSpec directory structure', async () => {
    ...
    it('should create config.yaml with default schema', async () => {
      ...
      const configPath = path.join(testDir, 'clearspec', 'config.yaml');
    ...
    it('should create skills in Cursor skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.cursor', 'skills', 'clearspec-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should create skills in Windsurf skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.windsurf', 'skills', 'clearspec-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should support Kimi CLI as an adapterless skills-only tool', async () => {
      ...
      const skillFile = path.join(testDir, '.kimi', 'skills', 'clearspec-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    ...
    it('should create skills for multiple tools at once', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'clearspec-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'clearspec-explore', 'SKILL.md');
    ...
    it('should select all tools with --tools all option', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'clearspec-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'clearspec-explore', 'SKILL.md');
      const windsurfSkill = path.join(testDir, '.windsurf', 'skills', 'clearspec-explore', 'SKILL.md');
    ...
    it('should skip tool configuration with --tools none option', async () => {
      ...
      // Should create ClearSpec structure but no skills
      const clearspecPath = path.join(testDir, 'clearspec');
      expect(await directoryExists(clearspecPath)).toBe(true);
    ...
    it('should not create config.yaml if it already exists', async () => {
      ...
      const clearspecDir = path.join(testDir, 'clearspec');
      await fs.mkdir(clearspecDir, { recursive: true });
      const configPath = path.join(clearspecDir, 'config.yaml');
    ...
    it('should handle non-existent target directory', async () => {
      ...
      const clearspecPath = path.join(newDir, 'clearspec');
      expect(await directoryExists(clearspecPath)).toBe(true);
    });
    ...
    it('should refresh skills on re-run for the same tool', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'clearspec-explore', 'SKILL.md');
  ...
  describe('skill content validation', () => {
    it('should generate valid SKILL.md with YAML frontmatter', async () => {
      ...
      expect(content).toContain('name: clearspec-explore');
      expect(content).toContain('description:');
    ...
    it('should include propose skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'clearspec-propose', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: clearspec-propose');
    });
    ...
    it('should include apply-change skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'clearspec-apply-change', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: clearspec-apply-change');
    });
  ...
  describe('command generation', () => {
    it('should generate Claude Code commands with correct format', async () => {
      ...
      const cmdFile = path.join(testDir, '.claude', 'commands', 'clsx', 'explore.md');
    ...
    it('should generate Cursor commands with correct format', async () => {
      ...
      const cmdFile = path.join(testDir, '.cursor', 'commands', 'clsx-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
      ...
      vi.spyOn(fs, 'writeFile').mockImplementation(
        async (filePath: any, ...args: any[]) => {
          if (
            ...
            filePath.includes('.clearspec-test-')
          ) {
  ...
  describe('tool-specific adapters', () => {
    it('should generate Gemini CLI commands as TOML files', async () => {
      ...
      const cmdFile = path.join(testDir, '.gemini', 'commands', 'clsx', 'explore.toml');
      expect(await fileExists(cmdFile)).toBe(true);
    ...
    it('should generate Windsurf commands', async () => {
      ...
      const cmdFile = path.join(testDir, '.windsurf', 'workflows', 'clsx-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
    ...
    it('should generate Continue prompt files', async () => {
      ...
      const cmdFile = path.join(testDir, '.continue', 'prompts', 'clsx-explore.prompt');
      expect(await fileExists(cmdFile)).toBe(true);

      const content = await fs.readFile(cmdFile, 'utf-8');
      expect(content).toContain('name: clsx-explore');
    ...
    it('should generate Cline workflow files', async () => {
      ...
      const cmdFile = path.join(testDir, '.clinerules', 'workflows', 'clsx-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
    ...
    it('should generate GitHub Copilot prompt files', async () => {
      ...
      const cmdFile = path.join(testDir, '.github', 'prompts', 'clsx-explore.prompt.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
  });
});
...
describe('InitCommand - profile and detection features', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `clearspec-init-profile-test-${Date.now()}`);
    await fs.mkdir(testDir, { recursive: true });
    originalEnv = { ...process.env };
    // Use a temp dir for global config to avoid polluting real config
    configTempDir = path.join(os.tmpdir(), `clearspec-config-test-${Date.now()}`);
    await fs.mkdir(configTempDir, { recursive: true });
    process.env.XDG_CONFIG_HOME = configTempDir;
    vi.spyOn(console, 'log').mockImplementation(() => {});
    confirmMock.mockReset();
  ...
  it('should use --profile flag to override global config', async () => {
    ...
    const proposeSkill = path.join(testDir, '.claude', 'skills', 'clearspec-propose', 'SKILL.md');
    expect(await fileExists(proposeSkill)).toBe(true);

    // Non-core skills (from the custom profile) should NOT be created
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'clearspec-new-change', 'SKILL.md');
  ...
  it('should auto-cleanup legacy artifacts in non-interactive mode without --force', async () => {
    ...
    await fs.writeFile(path.join(legacyDir, 'clsx-propose.md'), 'legacy content');
    ...
    expect(await fileExists(path.join(legacyDir, 'clsx-propose.md'))).toBe(false);
  ...
  it('should preselect configured tools but not directory-detected tools in extend mode', async () => {
    // Simulate existing ClearSpec project (extend mode).
    await fs.mkdir(path.join(testDir, 'clearspec'), { recursive: true });

    // Configured with ClearSpec
    const claudeSkillDir = path.join(testDir, '.claude', 'skills', 'clearspec-explore');
    await fs.mkdir(claudeSkillDir, { recursive: true });
    await fs.writeFile(path.join(claudeSkillDir, 'SKILL.md'), 'configured');

    // Directory detected only (not configured with ClearSpec)
  ...
  it('should preselect detected tools for first-time interactive setup', async () => {
    // First-time init: no clearspec/ directory and no configured ClearSpec skills.
    await fs.mkdir(path.join(testDir, '.github'), { recursive: true });
    await fs.writeFile(path.join(testDir, '.github', 'copilot-instructions.md'), '');
  ...
  it('should respect custom profile from global config', async () => {
    ...
    const exploreSkill = path.join(testDir, '.claude', 'skills', 'clearspec-explore', 'SKILL.md');
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'clearspec-new-change', 'SKILL.md');
  ...
  it('should migrate commands-only extend mode to custom profile without injecting propose', async () => {
    await fs.mkdir(path.join(testDir, 'clearspec'), { recursive: true });
    await fs.mkdir(path.join(testDir, '.claude', 'commands', 'clsx'), { recursive: true });
    await fs.writeFile(path.join(testDir, '.claude', 'commands', 'clsx', 'explore.md'), '# explore\n');
    ...
    const exploreCommand = path.join(testDir, '.claude', 'commands', 'clsx', 'explore.md');
    const proposeCommand = path.join(testDir, '.claude', 'commands', 'clsx', 'propose.md');
