...
describe('artifact-workflow CLI commands', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'clearspec-artifact-workflow-'));
    changesDir = path.join(tempDir, 'clearspec', 'changes');
  ...
  describe('status command', () => {
    ...
    it('exits gracefully when no changes exist', async () => {
      ...
      expect(result.stdout).toContain('clearspec new change');
    });
  ...
  describe('new change command', () => {
    ...
    it('creates workspace-planning changes under the workspace root without touching linked repos', async () => {
      ...
      await fs.mkdir(path.join(api, 'clearspec', 'specs'), { recursive: true });
      ...
      const metadata = await fs.readFile(path.join(changeDir, '.clearspec.yaml'), 'utf-8');
      ...
      await expect(fs.stat(path.join(api, 'clearspec', 'changes'))).rejects.toMatchObject({
        code: 'ENOENT',
      });
    });
  ...
  describe('instructions apply command', () => {
    ...
    it('resolves single-star glob artifacts consistently between status and apply', async () => {
      const schemaDir = path.join(tempDir, 'clearspec', 'schemas', 'glob-test');
...
`
      ...
      await fs.mkdir(path.dirname(specPath), { recursive: true });
      await fs.writeFile(path.join(changeDir, '.clearspec.yaml'), 'schema: glob-test\n');
    ...
    it('fallback: requires all artifacts when schema has no apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'clearspec', 'schemas', 'no-apply');
...
`;
    ...
    it('fallback: ready when all artifacts exist for schema without apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'clearspec', 'schemas', 'no-apply-full');
      const templatesDir = path.join(noApplySchemaDir, 'templates');
      await fs.mkdir(templatesDir, { recursive: true });
...
`;
  ...
  describe('experimental command (deprecated alias for init)', () => {
    ...
    it('creates skills for Claude tool', async () => {
      ...
      const skillFile = path.join(tempDir, '.claude', 'skills', 'clearspec-explore', 'SKILL.md');
    ...
    it('creates skills for Cursor tool', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.cursor', 'skills', 'clearspec-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);

      // Verify commands were created with Cursor format
      const commandFile = path.join(tempDir, '.cursor', 'commands', 'clsx-explore.md');
    ...
    it('creates skills for Windsurf tool', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.windsurf', 'skills', 'clearspec-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);
    });
  });
  ...
  describe('project config integration', () => {
    describe('new change uses config schema', () => {
      it('creates change with schema from project config', async () => {
        ...
        // Note: changesDir is already at tempDir/clearspec/changes (created in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'clearspec', 'config.yaml'),
          'schema: spec-driven\n'
        );
        ...
        const metadataPath = path.join(changesDir, 'test-change', '.clearspec.yaml');
      ...
      it('CLI schema overrides config schema', async () => {
        // Create project config with spec-driven schema
        // Note: clearspec directory already exists (from changesDir creation in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'clearspec', 'config.yaml'),
          'schema: spec-driven\n'
        );
        ...
        const metadataPath = path.join(changesDir, 'override-test', '.clearspec.yaml');
        const metadata = await fs.readFile(metadataPath, 'utf-8');
        expect(metadata).toContain('schema: spec-driven');
      }, 60000);
    });
...
`
    ...
    describe('backwards compatibility', () => {
      ...
      it('changes with metadata work without config file', async () => {
        ...
        await fs.writeFile(
          path.join(changeDir, '.clearspec.yaml'),
          'schema: spec-driven\ncreated: "2025-01-05"\n'
        );
