...
describe('config command integration', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `clearspec-config-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('should use XDG_CONFIG_HOME for config path', async () => {
    ...
    expect(configPath).toBe(path.join(tempDir, 'clearspec', 'config.json'));
  });
...
describe('config command shell completion registry', () => {
  it('should have config command in registry', async () => {
    ...
    expect(configCmd?.description).toBe('View and modify global ClearSpec configuration');
  });
...
describe('config profile command', () => {
  ...
  beforeEach(() => {
    tempDir = path.join(os.tmpdir(), `clearspec-profile-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    fs.mkdirSync(tempDir, { recursive: true });
    originalEnv = { ...process.env };
    process.env.XDG_CONFIG_HOME = tempDir;
  });
  ...
  it('core preset should set profile to core and preserve delivery', async () => {
    ...
    expect(result.workflows).toEqual(['propose', 'explore', 'apply', 'sync', 'archive', 'check-readiness', 'deep-review', 'discover']);
  });
