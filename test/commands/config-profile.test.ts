...
async function runConfigCommand(args: string[]): Promise<void> {
  ...
  await program.parseAsync(['node', 'clearspec', 'config', ...args]);
}
...
describe('deriveProfileFromWorkflowSelection', () => {
  ...
  it('returns core when selection has exactly core workflows in different order', async () => {
    const { deriveProfileFromWorkflowSelection } = await import('../../src/commands/config.js');
    expect(deriveProfileFromWorkflowSelection(['discover', 'deep-review', 'check-readiness', 'archive', 'sync', 'apply', 'explore', 'propose'])).toBe('core');
  });
});
...
describe('config profile interactive flow', () => {
  ...
  function setupDriftedProjectArtifacts(projectDir: string): void {
    fs.mkdirSync(path.join(projectDir, 'clearspec'), { recursive: true });
    const exploreSkillPath = path.join(projectDir, '.claude', 'skills', 'clearspec-explore', 'SKILL.md');
    fs.mkdirSync(path.dirname(exploreSkillPath), { recursive: true });
    fs.writeFileSync(exploreSkillPath, 'name: clearspec-explore\n', 'utf-8');
  }
  ...
  function setupSyncedCoreBothArtifacts(projectDir: string): void {
    ...
    const coreCommands = ['propose', 'explore', 'apply', 'sync', 'archive', 'check-readiness', 'deep-review', 'discover'];
    for (const commandId of coreCommands) {
      const commandPath = path.join(projectDir, '.claude', 'commands', 'clsx', `${commandId}.md`);
  ...
  function addExtraVerifyWorkflowArtifacts(projectDir: string): void {
    const verifySkillPath = path.join(projectDir, '.claude', 'skills', 'clearspec-verify-change', 'SKILL.md');
    fs.mkdirSync(path.dirname(verifySkillPath), { recursive: true });
    fs.writeFileSync(verifySkillPath, 'name: clearspec-verify-change\n', 'utf-8');

    const verifyCommandPath = path.join(projectDir, '.claude', 'commands', 'clsx', 'verify.md');
  ...
  function setupWorkspaceState(
    ...
    const metadataDir = path.join(workspaceRoot, '.clearspec-workspace');
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `clearspec-config-profile-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('selecting current values only should be a no-op and should not ask apply', async () => {
    ...
    fs.mkdirSync(path.join(tempDir, 'clearspec'), { recursive: true });
    select.mockResolvedValueOnce('delivery');
  ...
  it('changed config should ask to apply to the current workspace and print workspace guidance when declined', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('Config updated. Run `clearspec workspace update` to apply it to workspace-local skills.');
  });
  ...
  it('confirmed workspace apply should run workspace update instead of repo-local update', async () => {
    ...
    expect(execSync).toHaveBeenCalledWith('npx clearspec workspace update', {
      stdio: 'inherit',
      cwd: process.cwd(),
    });
    expect(execSync).not.toHaveBeenCalledWith('npx clearspec update', expect.anything());
  });
  ...
  it('no-op inside a workspace should warn when workspace skills drift', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('clearspec workspace update'));
  });
  ...
  it('core preset should preserve delivery setting', async () => {
    ...
    expect(config.workflows).toEqual(['propose', 'explore', 'apply', 'sync', 'archive', 'check-readiness', 'deep-review', 'discover']);
